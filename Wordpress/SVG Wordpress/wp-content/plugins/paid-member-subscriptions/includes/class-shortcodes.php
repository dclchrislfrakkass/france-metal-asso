...
Class PMS_Shortcodes {
    ...
    public static function member_account( $args ) {
              ...
              <ul>
                  <?php foreach( $tabs as $slug => $name ) : ?>
                      <li class="pms-account-navigation-link pms-account-navigation-link--<?php echo $slug; ?>">
                          <a class="<?php echo ( $active_tab == $slug ? 'pms-account-navigation-link--active' : '' ); ?>" href="<?php echo esc_url( ( $slug != 'logout' ? pms_account_get_tab_url( $slug, pms_get_page( 'account', true ) ) : wp_logout_url() ) ); ?>"><?php echo $name; ?></a>
                      </li>
                  <?php endforeach; ?>
              </ul>
            </nav>
        ...
        if ( $active_tab == 'subscriptions' ) {
            ...
            if( !$member->is_member() ) {
                $message = '<p>' . __( 'You do not have any subscriptions attached to your account.', 'paid-member-subscriptions' ) . '</p>';
            ...
            if( ! empty( $args['subscription_plans'] ) ) {
                ...
                if( ! empty( $member_subscriptions ) ) {
                    ...
                    foreach( $member_subscriptions as $member_subscription ) {
                        ...
                        if( !empty($member_subscription->status) && ( $member_subscription->status == 'active' || $member_subscription->status == 'canceled' ) && ! $time_expire ) {
                            $valid_subscriptions[] = $member_subscription->subscription_plan_id;
                        }

                    }

                    $common_subscriptions = array_intersect( $subscription_plans, $valid_subscriptions );
