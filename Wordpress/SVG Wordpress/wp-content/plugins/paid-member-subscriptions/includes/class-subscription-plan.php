...
Class PMS_Subscription_Plan {
    ...
    public function init( $id_or_post ) {
        ...
        $this->type = !empty( $post_meta_subscription['pms_subscription_plan_type'][0] ) ? $post_meta_subscription['pms_subscription_plan_type'][0] : '';
    ...
    /**
     * Returns the expiration date for Fixed Expiration Date plans
     *
     * @param bool $timestamp
     *
     * @return string
     *
     */
    public function get_fixed_expiration_date( $timestamp = false ) {

        if ( empty( $this->fixed_expiration_date) )
            return '';

        $time = strtotime( $this->fixed_expiration_date );
