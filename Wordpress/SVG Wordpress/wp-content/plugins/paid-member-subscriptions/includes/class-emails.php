    ...
    static function send_registration_email( $user_data = array() ) {
        ...
        if( ! ( defined( 'PROFILE_BUILDER' ) && PROFILE_BUILDER == 'Profile Builder Pro' && ! empty( $wppb_addonOptions['wppb_emailCustomizer'] ) && $wppb_addonOptions['wppb_emailCustomizer'] == 'show' ) ) {
            ...
            if ( isset( $settings[ 'register_is_enabled' ] ) )
                PMS_Emails::pms_mail( 'user', 'register', $user_data['user_id'], $user_data['subscriptions'][0] );

            if ( isset( $settings[ 'register_admin_is_enabled' ] ) )
                PMS_Emails::pms_mail( 'admin', 'register', $user_data['user_id'], $user_data['subscriptions'][0] );
