...
add_action( 'wbcr_factory_bootstrap_410_plugin_created', 'wbcr_factory_bootstrap_410_plugin_created' );

/**
 * @param Wbcr_Factory410_Plugin $plugin
 */
function wbcr_factory_bootstrap_410_plugin_created( $plugin ) {
	$manager = new Wbcr_FactoryBootstrap410_Manager( $plugin );
	$plugin->setBootstap( $manager );
}

if ( ! class_exists( 'Wbcr_FactoryBootstrap410_Manager' ) ) {
	 ...
	 * @since 3.2.0
	 */
	class Wbcr_FactoryBootstrap410_Manager {
		...
		public function loadAssets( $hook ) {
			...
			do_action( 'wbcr_factory_410_bootstrap_enqueue_scripts', $hook );
			do_action( 'wbcr_factory_410_bootstrap_enqueue_scripts_' . $this->plugin->getPluginName(), $hook );
			...
			if ( $color_name !== 'fresh' ) {
				if ( file_exists( FACTORY_BOOTSTRAP_410_DIR . '/assets/flat/css/bootstrap.' . $color_name . '.css' ) ) {
					wp_enqueue_style( 'wbcr-factory-bootstrap-410-colors', FACTORY_BOOTSTRAP_410_URL . '/assets/flat/css/bootstrap.' . $color_name . '.css' );
				}
			}
            ...
            <script>
				...
				if( !window.factory.factoryBootstrap410 ) {
					window.factory.factoryBootstrap410 = {};
				}
				window.factory.factoryBootstrap410.colors = {
		...
		protected function enqueueScripts( array $sripts, $type = 'js', array $dependencies ) {
			...
			$cache_dir_path = FACTORY_BOOTSTRAP_410_DIR . '/assets/cache/';
			$cache_dir_url  = FACTORY_BOOTSTRAP_410_URL . '/assets/cache/';
			...
			if ( $cache_enable && file_exists( $cache_filepath ) ) {
				...
				if ( ! file_exists( $cache_dir_path ) ) {
					if ( @mkdir( $cache_dir_path, 0755 ) && is_writable( $cache_dir_path ) ) {
				...
				foreach ( $sripts as $script_to_load ) {
					...
					if ( $cache_enable && $cache_dir_exists ) {
						$fname = FACTORY_BOOTSTRAP_410_DIR . "/assets/$type-min/$script_to_load.min." . $type;
						...
						if ( $type == 'js' ) {
							wp_enqueue_script( md5( $script_to_load ), FACTORY_BOOTSTRAP_410_URL . "/assets/$type-min/$script_to_load.min." . $type, $is_first ? $dependencies : false, $this->plugin->getPluginVersion() );
						} else {
							wp_enqueue_style( md5( $script_to_load ), FACTORY_BOOTSTRAP_410_URL . "/assets/$type-min/$script_to_load.min." . $type, array(), $this->plugin->getPluginVersion() );
						}
						$is_first = false;
					}
				}
