...
<div class="wrap">
    ...
    <h2><?php ($action == 'edit_payment' ) ? printf( __( 'Payment #%s', 'paid-member-subscriptions' ), $payment_id ) : _e('Add New Payment', 'paid-member-subscriptions');  ?></h2>
    ...
    <form id="pms-form-<?php echo ( $action == 'edit_payment' ? 'edit' : 'add' ); ?>-payment" class="pms-form" method="POST" action="<?php echo $url; ?>">
        ...
        <div class="pms-form-field-wrapper">
            ...
            <select id="pms-payment-type" name="pms-payment-type" class="widefat">
                ...
                if ( $action == 'add_payment' ) {
                    echo '<option selected="selected" value="manual_payment">'. __('Manual Payment', 'paid-member-subscriptions' ) .'</option>';
                }
                else {
        ...
        <?php
        if ( $action == 'edit_payment' )
            do_action( 'pms_payment_edit_form_field' );
        else
