...
Class PMS_Form_Handler {
     ...
     * 2. Verifies if the selected subscription plan's settings match the payment gateway's supports features
     * 3. Verifies the custom fields of the selected payment gateway
     *
    ...
    public static function get_redirect_url() {
        ...
        switch( $location ) {
            ...
            case 'register':
                ...
                $url = ( isset( $pms_settings['register_success_page'] ) && $pms_settings['register_success_page'] != -1 ? get_permalink( trim( $pms_settings['register_success_page'] ) ) : '' );
    ...
    public static function process_checkout( $user_data = array() ) {
        ...
        if( $form_location == 'register' && empty( $user_data['user_id'] ) ) {
            ...
            $user_data['user_id'] = self::register_user( $user_data );
        ...
        if( $has_trial ) {
            ...
            $subscription_data['expiration_date'] = date( 'Y-m-d H:i:s', strtotime( "+" . $subscription_plan->trial_duration . ' ' . $subscription_plan->trial_duration_unit, $subscription_plan->get_expiration_date( true ) ) );
