...
class PMS_Add_General_Notices{
    ...
    function add_admin_notice() {
        ...
        if ( current_user_can( 'manage_options' ) ){
            ...
            if ( ! get_user_meta($user_id, $this->notificationId.'_dismiss_notification' ) ) {
                echo $finalMessage = apply_filters($this->notificationId.'_notification_message','<div class="notice '. $this->notificationClass .'" ><p>'.$this->notificationMessage.'</p></div>', $this->notificationMessage);
            }
    ...
    function remove_other_plugin_notices() {
        ...
        if ( isset( $_GET['page'] ) && ( $_GET['page'] == 'pms-basic-info-page' || $_GET['page'] == 'pms-addons-page' ) ) {
